...
def lone_sum(a, b, c):
  soma = 0
  indices = [i for i, x in enumerate([a,b,c]) if x == a]
  indicesb = [i for i, x in enumerate([a, b, c]) if x == b]
  indicesc = [i for i, x in enumerate([a, b, c]) if x == c]
  if len(indices)>1:
    pass
  else:
    soma += a
...
'''lista = [a, b, c]
  for c in range(0, len(lista)-1):
    for c2 in range(c+1, len(lista)-1):
      if lista[c] == lista[c2]:
        print('if', lista[c], lista[c2])
        lista.remove(lista[c])
        lista.remove(lista[c2])
  if len(lista) == 1:
    return 0
  print(lista)
  return sum(lista)'''
...
def end_other(a, b):
  ...
  if a == 'abc' and b == 'abc':
    return True
  if b[(len(b)-3):len(b)] in a[0:len(a)]:
    return True
  if a[(len(b)-3):len(a)] in b[0:len(b)]:
    return True
  return False
# I. count_evens
...
def fila_tijolos(peq, gra, meta):
  return peq >= meta % 5 and peq + 5 * gra >= meta
